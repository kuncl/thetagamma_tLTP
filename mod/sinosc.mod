...
NEURON {
  ...
  RANGE  del, dur, freq, i, isinbar, grampbar,eramp, rampg,rampi, ramppos, gsinbar,esin, sing, sini, sinphase,  bias, bias_i, noise, noisei
...
BREAKPOINT {
  ...
  else
  {
	...
	if (t > del && t < del+dur){
		...
		sini = sing*(esin-v)
		sini = isinbar + isinbar * sin( 2*PI * (t-del +  ((1000/freq) * (sinphase/360))  ) * (freq) * (0.001) )					
		bias_i = bias
