...
NEURON {
	...
	USEION ca READ ica, cai WRITE cai	
        RANGE ca,  depth,cainf,taur, irest
}
...
PARAMETER {
	...
	cainf	= 100e-6(mM)
	cai		(mM)
	kb = 18					: buffer ratio from Sabatini 2002, Value of 20 when buffer is included
...
DERIVATIVE state { 
	...
	drive_channel =  - (10000) * ( ica - irest) / (2 * FARADAY * depth)
	if (drive_channel <= 0.) { drive_channel = 0.  }   : cannot pump inward 
         
	: ca' = drive_channel + (cainf-ca)/taur*7
    ca' = drive_channel/kb + (cainf -ca)/taur*7
	cai = ca
}
