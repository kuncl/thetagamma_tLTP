...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 cagk /home2/hjang/Research/AD_OPTO_GAMMA/Evolution_Algorithm_Fit/neuron_code_tLTP/mod/x86_64/cagk_morse.mod\n");
...
static int  rate (  double _lv , double _lc ) {
   a = alp ( _threadargscomma_ _lv , _lc ) ;
   tau = 1.0 / ( a + bet ( _threadargscomma_ _lv , _lc ) ) ;
   oinf = a * tau ;
    return 0; }
 ...
 v=_v;
{
  ...
  ek = _ion_ek;
 { error =  state();
 if(error){fprintf(stderr,"at line 62 in file cagk_morse.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
