...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 cad /home2/hjang/Research/AD_OPTO_GAMMA/Evolution_Algorithm_Fit/neuron_code_tLTP/mod/x86_64/cad.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   drive_channel = - ( 10000.0 ) * ( ica - irest ) / ( 2.0 * FARADAY * depth ) ;
   if ( drive_channel <= 0. ) {
     drive_channel = 0. ;
     }
   Dca = drive_channel / kb + ( cainf - ca ) / taur * 7.0 ;
   cai = ca ;
   }
 return _reset;
}
 ...
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   ...
   if ( drive_channel <= 0. ) {
    ...
    ca = ca + (1. - exp(dt*(( ( ( ( - 1.0 ) ) ) / taur )*( 7.0 ))))*(- ( ( drive_channel ) / kb + ( ( ( cainf ) ) / taur )*( 7.0 ) ) / ( ( ( ( ( - 1.0 ) ) ) / taur )*( 7.0 ) ) - ca) ;
   cai = ca ;
   }
  return 0;
}
